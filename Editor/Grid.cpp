...
CGrid::CGrid()
{
  ...
  ZeroMemory(&m_whiteMaterial, sizeof(D3DMATERIAL8));
  m_whiteMaterial.Emissive.r = 0.55f;
  m_whiteMaterial.Emissive.g = 0.55f;
  m_whiteMaterial.Emissive.b = 0.55f;
}
...
void CGrid::Render(IDirect3DDevice8* device)
{
  ...
  if(buffer != NULL)
  {
    device->SetMaterial(&m_whiteMaterial);
